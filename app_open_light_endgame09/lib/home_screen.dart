...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _onConnected(String clientKey) {
    ...
    // Đảm bảo client không null và chưa có listener nào được gắn vào
    if (client != null && _clientUpdateSubscriptions[clientKey] == null) {
      ...
      if (client.updates != null) {
        print('MQTT Client [$clientKey] Setting up message listener...');
        _clientUpdateSubscriptions[clientKey] = client.updates!
            .listen((List<MqttReceivedMessage<MqttMessage?>>? c) {
          _handleMqttMessage(c, clientKey);
        });
      } else {
        // Ghi log hoặc xử lý trường hợp stream updates bị null sau khi kết nối
        print(
            'Error: MQTT client updates stream is null for $clientKey even after connection.');
      }
    } else if (client == null) {
      print('Error: MQTT client is null in _onConnected for $clientKey');
    } else {
      print('MQTT Client [$clientKey] message listener already exists.');
    }
