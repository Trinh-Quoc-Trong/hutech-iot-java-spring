...
import 'package:mqtt_client/mqtt_server_client.dart'; // Cho Native
import 'package:mqtt_client/mqtt_browser_client.dart'; // Cho Web
import 'package:mqtt_client/mqtt_client.dart'; // Import đầy đủ các thành phần chung
import 'dart:convert'; // Thêm import để sử dụng jsonDecode
import 'package:mobile_scanner/mobile_scanner.dart'; // Import QR Scanner
import 'package:flutter/foundation.dart' show kIsWeb; // Thêm import kIsWeb
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(
        maxCrossAxisExtent: 180.0, // Đặt chiều rộng tối đa cho mỗi item
        crossAxisSpacing: 16.0, // Khoảng cách ngang
        mainAxisSpacing: 16.0, // Khoảng cách dọc
        childAspectRatio:
            1.0, // Tỷ lệ thẻ (width / height) - giữ nguyên hoặc điều chỉnh nếu cần
      ),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Sử dụng MqttClient làm kiểu cơ sở thay vì MqttServerClient
  final Map<String, MqttClient> _mqttClients = {};
  ...
  void initState() {
    ...
    // --- Thêm thiết bị mặc định với broker động ---
    final String currentBroker = kIsWeb ? "localhost" : "192.168.100.40";
    // Giữ nguyên port 1883, nhưng bạn có thể cần đổi nếu cổng WebSocket trên localhost khác
    const int currentPort = 1883;
    const String currentTopic = "/test/topic1";

    final defaultDevice = DeviceInfo(
      // Cập nhật ID để phản ánh broker và port hiện tại
      id: "default_\${currentBroker}:\${currentPort}_\${currentTopic.replaceAll('/', '_')}",
      name: "Đèn Mặc Định", // Tên ví dụ
      broker: currentBroker, // Sử dụng broker đã xác định
      port: currentPort, // Sử dụng port đã xác định
      topic: currentTopic,
  ...
  Future<void> _connectAndSubscribeDevice(DeviceInfo device) async {
    ...
    MqttClient client; // Sử dụng kiểu cơ sở MqttClient
    ...
    if (_mqttClients.containsKey(clientKey)) {
      ...
      // --- START: Chọn Client phù hợp với nền tảng ---
      if (kIsWeb) {
        // Tự xây dựng URI WebSocket đầy đủ với scheme ws://
        final String webSocketUrl = 'ws://${device.broker}:${device.port}';
        print('MQTT: Attempting WebSocket connection to: $webSocketUrl');
        // Sử dụng constructor mặc định của MqttBrowserClient với URI đầy đủ
        client = MqttBrowserClient(webSocketUrl, device.clientIdentifier);
        // Không cần set client.port vì nó đã là một phần của URI
        // client.useWebSocket đã là true mặc định cho MqttBrowserClient
      } else {
        client = MqttServerClient(device.broker, device.clientIdentifier);
        // MqttServerClient mặc định dùng TCP
        client.port = device.port;
        print(
            'MQTT: Using MqttServerClient (TCP) for native on port ${device.port}');
      }
      // --- END: Chọn Client phù hợp với nền tảng ---
      ...
      body: Container(
        ...
        child: allDevices.isEmpty
            ...
            : GridView.builder(
                ...
                gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(
                  maxCrossAxisExtent:
                      180.0, // Đặt chiều rộng tối đa cho mỗi item
                  crossAxisSpacing: 16.0, // Khoảng cách ngang
                  mainAxisSpacing: 16.0, // Khoảng cách dọc
                  childAspectRatio:
                      1.0, // Tỷ lệ thẻ (width / height) - giữ nguyên hoặc điều chỉnh nếu cần
