...
import 'package:speech_to_text/speech_recognition_result.dart'; // Import SpeechRecognitionResult
import 'package:speech_to_text/speech_recognition_error.dart'; // Import SpeechRecognitionError
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  bool _isListening = false; // Trạng thái đang lắng nghe của speech_to_text
  bool _awaitingCommand = false; // Chờ lệnh sau khi nói "ok"
  String _lastWords = ''; // Từ cuối cùng được nhận dạng
  Timer? _restartListenTimer; // Timer để khởi động lại lắng nghe
  // --- End Speech to Text ---
  ...
  @override
  void dispose() {
    // Hủy các timer và dừng lắng nghe/ngắt kết nối MQTT
  ...
  void _initSpeech() async {
    try {
      ...
      if (_speechEnabled) {
        ...
        // Bắt đầu lắng nghe ngay lập tức nếu khởi tạo thành công
        _startListening();
      } else {
  ...
  void _startListening() {
    _restartListenTimer?.cancel(); // Hủy timer cũ nếu có
    if (!_speechEnabled || _speechToText.isListening) {
      print("Speech not enabled or already listening. Cannot start listening.");
      // Nếu đang lắng nghe rồi thì không cần làm gì thêm
    ...
    _speechToText.listen(
      ...
      listenFor: const Duration(minutes: 5), // Nghe trong thời gian dài
      pauseFor: const Duration(seconds: 5), // Tự động dừng sau 5s im lặng
      partialResults: true, // Nhận kết quả tạm thời
      localeId: 'vi_VN', // Chỉ định ngôn ngữ tiếng Việt
      cancelOnError: true, // Tự động hủy nếu có lỗi
      listenMode: ListenMode.dictation, // Chế độ nhận dạng liên tục hơn
    );
    if (mounted) {
      setState(() {
        _isListening = true; // Cập nhật UI ngay
      });
    }
  }
  ...
  // --- Dừng lắng nghe ---
  void _stopListening() {
    _restartListenTimer?.cancel(); // Hủy timer nếu đang chờ restart
    ...
    if (mounted) {
      setState(() {
        _isListening = false;
        _awaitingCommand = false; // Reset khi dừng chủ động
      });
    }
  }

  // --- Callback khi kết quả nhận dạng thay đổi ---
  void _onSpeechResult(SpeechRecognitionResult result) {
    ...
    setState(() {
      _lastWords = recognizedWords; // Cập nhật để hiển thị (nếu muốn)
    });

    // Chỉ xử lý nếu có kết quả cuối cùng HOẶC kết quả tạm thời chứa từ khóa/lệnh
    if (result.finalResult ||
        _awaitingCommand ||
        recognizedWords.contains('ok')) {
      if (_awaitingCommand) {
        // Đã nghe "ok", chờ lệnh "bật" hoặc "tắt"
        if (recognizedWords.contains("bật")) {
          print("Voice Command: Bật");
          _sendCommandToFirstDevice('1'); // Gửi lệnh bật
          _resetAndRestartListening(); // Reset và lắng nghe lại cho "ok"
        } else if (recognizedWords.contains("tắt")) {
          print("Voice Command: Tắt");
          _sendCommandToFirstDevice('0'); // Gửi lệnh tắt
          _resetAndRestartListening(); // Reset và lắng nghe lại cho "ok"
        } else if (result.finalResult) {
          // Nếu là kết quả cuối cùng mà không có lệnh hợp lệ sau "ok"
          print("Final result after 'ok', but no valid command. Resetting.");
          _resetAndRestartListening(
              resetAwaiting: true); // Reset trạng thái chờ
        }
      } else {
        // Chưa nghe "ok", tìm từ khóa "ok"
        if (recognizedWords.contains("ok")) {
          print("Keyword 'ok' detected!");
          setState(() {
            _awaitingCommand = true; // Đặt cờ chờ lệnh
          });
          // Không reset listening ở đây, tiếp tục nghe lệnh
        }
      }
    }
  }

// --- Hàm gửi lệnh MQTT tới thiết bị đầu tiên ---
  void _sendCommandToFirstDevice(String message) {
    if (allDevices.isNotEmpty) {
      final firstDevice = allDevices.first;
      _publishMessage(
          firstDevice.topic, message, firstDevice.broker, firstDevice.port);
    } else {
      print("No devices available to send command.");
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Không có thiết bị nào để điều khiển.')),
        );
      }
    }
  }

// --- Hàm reset và khởi động lại lắng nghe ---
  void _resetAndRestartListening({bool resetAwaiting = true}) {
    print("Resetting and restarting listener...");
    if (resetAwaiting) {
      _awaitingCommand = false;
    }
    _lastWords = ''; // Clear last words
    // Dừng ngay lập tức và khởi động lại sau một khoảng trễ ngắn
    // để đảm bảo trạng thái được cập nhật đúng
    _speechToText.stop().then((_) {
      // Dùng Timer để tránh gọi listen quá nhanh sau khi stop
      _restartListenTimer?.cancel();
      _restartListenTimer = Timer(const Duration(milliseconds: 200), () {
    ...
    if (mounted) {
      setState(() {
        _isListening = false; // Tạm thời set false khi đang restart
        if (resetAwaiting) _awaitingCommand = false;
      });
    }
  }

  // --- Callback khi trạng thái lắng nghe thay đổi ---
  void _onSpeechStatus(String status) {
    ...
    if (status == SpeechToText.notListeningStatus ||
      ...
      print("Listener stopped. Scheduling restart...");
      // Reset trạng thái chờ lệnh nếu listener dừng đột ngột
      if (mounted) {
        setState(() {
          _awaitingCommand = false;
        });
      }
      // Lên lịch khởi động lại sau một khoảng trễ ngắn để tránh vòng lặp lỗi
      // và để cho phép xử lý kết quả cuối cùng nếu có
      _restartListenTimer?.cancel();
      _restartListenTimer = Timer(const Duration(milliseconds: 500), () {
        if (mounted && _speechEnabled && !_speechToText.isListening) {
          print("Executing scheduled restart...");
          _startListening();
        } else {
          print(
              "Scheduled restart cancelled (not mounted, speech disabled, or already listening).");
        }
      });
    }
  }

  // --- Callback khi có lỗi nhận dạng ---
  void _onSpeechError(SpeechRecognitionError error) {
    if (!mounted) return;
    print('Speech recognition error: ${error.errorMsg} - ${error.permanent}');
    setState(() {
      _isListening = false;
      _awaitingCommand = false; // Reset trạng thái khi có lỗi
      _lastWords = 'Lỗi: ${error.errorMsg}';
    });
    // Cân nhắc khởi động lại lắng nghe sau lỗi tạm thời?
    if (!error.permanent) {
      _restartListenTimer?.cancel();
      _restartListenTimer = Timer(const Duration(seconds: 1), () {
        if (mounted && _speechEnabled) {
          print("Attempting restart after temporary error...");
          _startListening();
        }
      });
    } else {
      setState(() {
        _speechEnabled = false; // Vô hiệu hóa nếu lỗi vĩnh viễn
        _lastWords = 'Lỗi vĩnh viễn, không thể nhận dạng giọng nói.';
      });
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          // --- Icon trạng thái lắng nghe giọng nói ---
          Padding(
            padding: const EdgeInsets.only(right: 8.0),
            child: Icon(
              _isListening ? Icons.mic : Icons.mic_off,
              color: _isListening
                  ? Theme.of(context).colorScheme.primary // Màu khi đang nghe
                  : Colors.grey, // Màu khi không nghe
              size: 24,
            ),
          ),
          // --- End Icon trạng thái lắng nghe giọng nói ---
