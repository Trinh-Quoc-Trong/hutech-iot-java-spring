...
#include "qrcodegen.h" // Thêm thư viện QR Code generator  
...
// Hàm tạo và in mã QR với ký tự Unicode (█)  
void generate_qr_code(const char *text) {  
    uint8_t qrcodeBytes[qrcodegen_BUFFER_LEN_MAX];  
    uint8_t tempBuffer[qrcodegen_BUFFER_LEN_MAX];  
    bool ok = qrcodegen_encodeText(text, tempBuffer, qrcodeBytes, qrcodegen_Ecc_LOW,  
    ...
    int size = qrcodegen_getSize(qrcodeBytes);  
    for (int y = 0; y < size; y++) {  
        for (int x = 0; x < size; x++) {  
            bool isBlack = qrcodegen_getModule(qrcodeBytes, x, y);  
            printf("%s", isBlack ? "██" : "  "); // Sử dụng ██ thay vì ##  
        }  
        printf("\n");  
    }  
}  
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {  
    if (event_base == WIFI_EVENT) {  
        ...
        ESP_LOGI(TAG, "Generated QR code for MQTT broker and topic:");  
